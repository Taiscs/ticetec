...
<head>
    ...
    <title>SyncPro - Dashboard Global</title>
    ...
    <script type="module">
        ...
        // Inicialização do Firebase (SIMULADO)
        ...
        const state = {
            ...
            view: 'admin', 
            userId: simulatedAdminId, 
            userEmail: simulatedAdminEmail, 
            isAuthReady: false,
            userRole: 'admin', 
            ...
            message: '',
            isSigningUp: false,
            currentBusinessId: null, // Para o modal
            
            // NOVO ESTADO PARA SINCRONIZAÇÃO
            businesses: [], 
            clientView: 'list', // 'list' ou 'register' - NOVO ESTADO
        ...
        state.businesses.push({
            id: 'b-2',
            ownerId: simulatedAdminId, // Empresa do próprio admin (simulada)
            name: 'Pizzaria Central',
            address: 'Rua Augusta, 500',
            phone: '(11) 98765-4321',
            website: 'www.pizzariacentral.com',
            description: 'Melhor pizza napolitana.',
            status: 'error',
            syncStatus: {
                google: { status: 'error', log: 'Falha na autenticação do Google.' },
                apple_maps: { status: 'processing', log: 'Em processamento.' },
                facebook: { status: 'error', log: 'Falha na autenticação do Google.' },
                bing: { status: 'error', log: 'Falha na autenticação do Google.' },
                foursquare: { status: 'error', log: 'Falha na autenticação do Google.' },
                kekanto: { status: 'processing', log: 'Em processamento.' },
                apontador: { status: 'success', log: 'Publicado em 2025-09-27' },
            }
        });
        ...
        // Simula o processo de sincronização para uma empresa
        window.simulateSyncProcess = (businessId) => {
            ...
            // Simula o tempo de processamento e resultados (2.5s)
            setTimeout(() => {
                let successCount = 0;
        ...
        // --- FUNÇÃO PARA MUDAR A VIEW DO CLIENTE ---
        window.setClientView = (view) => {
            ...
            appDiv.innerHTML = `
                <div class="min-h-screen bg-gray-50 p-4 sm:p-8">
                    <header class="flex flex-col sm:flex-row justify-between items-center py-4 px-6 bg-white shadow-lg rounded-xl mb-6">
                        <h1 class="text-2xl font-bold text-blue-600">SyncPro - Sincronização de Diretórios</h1>
                        ${state.view !== 'login' ? `
        ...
        // View de Login/Cadastro (Não acessível na inicialização, mas mantido para LogOut)
        ...
        // --- CLIENTE VIEW: FORMULÁRIO ---

        function BusinessForm() {
            return `
                <div class="p-6 bg-white rounded-xl card h-fit">
                    <h2 class="text-2xl font-bold text-gray-800 mb-4">Formulário de Cadastro de Empresa</h2>
        ...
        function BusinessList() {
            ...
            return `
                <div class="p-6 bg-white rounded-xl card">
                    <h2 class="text-2xl font-bold text-blue-700 mb-4">Dashboard de Sincronização</h2>
                    <p class="text-gray-600 mb-6">Acompanhe o status de sincronização em tempo real de todas as suas empresas.</p>
                    <div class="space-y-4">
                        ${myBusinesses.length > 0 ? myBusinesses.map(b => {
                            const statusInfo = getStatusClasses(b.status);
                            return `
                                <div class="bg-blue-50 p-4 rounded-lg border-l-4 ${statusInfo.color} shadow-md flex justify-between items-start flex-wrap gap-3">
                            ...
                            <div class="p-6 bg-yellow-50 border-l-4 border-yellow-400 rounded-lg">
                                <p class="text-yellow-800 font-medium">Nenhuma empresa cadastrada. Clique na aba "Cadastrar Empresa" para começar a sincronizar!</p>
                            </div>
                        `}
                    </div>
                </div>
        ...
        // --- CLIENTE VIEW PRINCIPAL COM NAVEGAÇÃO ---

        function ClientDashboard(messageHtml) {
            
            const activeClass = (view) => state.clientView === view ? 'active' : 'text-gray-500 hover:text-blue-600';

            return `
                <div class="bg-white rounded-xl card">
                    <!-- Tabs de Navegação -->
                    <div class="flex border-b border-gray-200">
                        <button 
                            onclick="setClientView('list')" 
                            class="tab-btn ${activeClass('list')}">
                            Dashboard (Status de Sincronização)
                        </button>
                        <button 
                            onclick="setClientView('register')" 
                            class="tab-btn ${activeClass('register')}">
                            Cadastrar Empresa
                        </button>
                    </div>
                    ...
                    <div class="p-6">
                        ${messageHtml}
                        ${state.clientView === 'register' ? BusinessForm() : BusinessList()}
                    </div>
                </div>

                <!-- Modal de Detalhes de Sincronização (Visível apenas se houver um ID selecionado) -->
                ${SyncDetailsModal()}
            `;
        }
        ...
        function AdminDashboard(messageHtml) {
            // Dashboard mostrando status de sincronização em cada diretório
            ...
            return `
                <div class="p-6 bg-white rounded-xl card">
                    ...
                    <div class="mb-8 p-4 bg-blue-50 rounded-lg">
                        ...
                        <p class="text-sm text-gray-600">Relatório/Logs com Status de Envio.</p>
                    </div>

                    <!-- Dashboard de Sincronização por Diretório -->
                    <h3 class="text-xl font-semibold mb-4 text-gray-800">Status por Diretório: Visão de Alto Nível</h3>
                            ...
                            return `
                                <div class="bg-gray-50 p-3 rounded-lg border-l-4 ${statusInfo.color} shadow-sm flex justify-between items-center">
                                    <div class="flex-1">
                                        ...
                                        <p class="text-xs text-gray-500">Proprietário ID: ${b.ownerId.substring(0, 8)}... | ${b.address.substring(0, 30)}...</p>
                                    </div>
                                    <div class="text-right">
        ...
        window.handleSaveBusiness = (event) => {
            ...
            // Limpa o formulário e retorna para a lista de empresas
            event.target.reset();
        ...
        window.handleAuth = (event) => {
            ...
            // Login/Cadastro de Cliente (Simulado)
            else {
                state.userId = `client-${email.split('@')[0]}-${crypto.randomUUID().substring(0, 4)}`; 
                state.userEmail = email;
